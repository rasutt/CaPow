...
matLay =
  ...
  function(layinp, padding = "5px") {
    tagList(
      if (!is.null(layinp) && nrow(layinp) > 0) {
        # Loop over non-data rows and make an HTML table, a list of rows
        tab = NULL
        for (i in 1:nrow(layinp)) {
          # Loop over columns creating a list of cells representing one row
          currow = NULL
          for (j in 1:ncol(layinp)) {
            currow = c(currow, list(tags$td(layinp[i, j], style = paste0("padding:", padding))))
          }
          tab = c(tab, list(tags$tr(currow)))
        }
        tags$table(class = "matLay", tab)
      }, tags$script(
        '(function(){
...
matLayAc =
  ...
  function(colTypes = NULL, colIDs = NULL, colArgs = NULL, colNames = NULL, nrow = 1,
    ...
    # Loop over rows building output matrix
    outmat = NULL
    for(i in 1:nrow){
      # Loop over columns building one row
      currow = NULL
      for(j in 1:ncol){
        rowargs = colIDs[j]
        rowargs[rowargs != ""] = paste0(rowargs, sprintf(paste0("%0", ndigits, "d"), i))
        rowargs = c(list(id = rowargs), lapply(colArgs[[j]], function(x) rep(x, length = nrow)[i]))
    ...
    # Create HTML table from output matrix
    matLay(layinp = outmat)
  }
