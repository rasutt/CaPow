...
process.onepar.func <- function(res.clean, res.setup, partype, conf, collapse.single, impose.01limits){
        ...
        if((length(which.par.est) + length(which.par.piggy)>0)){
                ...
                for(cipar in parnames.for.ci){
                        ## CI coverage:
                        par.out$CIcover[par.out$parname==cipar] <- mean(ci.out[, paste0("cover.", cipar)])
                        ## median CI width:
...
popan.ci.plot <- function(res.raw, plotwhat=c("phi", "lambda", "N"), conf=0.95, collapse.single=T,
        ...
        onepanel.ciplot <- function(parnm, partyp, colpt, trueval){
                ...
                text((nres+1)/2, y.title, paste0(parnm, " :  True = ", round(trueval, round.val),
                                                 " ; CI = ", round(mean(cipar.df[, "cover"])*100), "%"), cex=cex.main)
...
multiproj.power.plot <- function(projects=c("P11", "P13"), partype="lambda", timelabels=NULL,
        ...
        one.timelabel.plot <- function(timel){
                ...
                for(i in 1:nproj){
                        ...
                        details.timel$CIbelow[i] <- powertab.proj["cib"] / sum(powertab.proj) * 100
                        details.timel$CIabove[i] <- powertab.proj["cia"] / sum(powertab.proj) * 100
...
checkModel.func <- function(model){
        ...
        if(sum(model$paramdf$timeopt)<2)
                return(list(checkOK=F,
                            ...
                            "Your model has NOT been saved. Untick the box, correct the error, and try again.")))
...
checkSim.func <- function(sim){
        ...
        if(sum(sim$paramdf$timeopt)<2)
                return(list(checkOK=F,
                            ...
                            "Your SimSet has NOT been saved. Untick the box, correct the error, and try again.")))
...
popan.sim.wrap <- function(projname=NULL, Nsim=1000, modelname=NULL, simname=NULL,
                           gradtolval=1e-4){
        ...
        for(sim in 1:Nsim){
                ...
                ## Generate data:
                dat.sim <- sim.func(N=proj.setup$SimN, simtable=proj.setup$SimTable)
                ## Fit model:
                res.sim <- try(popan.func(det.dat=dat.sim, setup.res=proj.setup, gradtol=gradtolval, printit=F))
...
popan.func <- function(det.dat, setup.res, gradtol=1e-4, printit=T){
        ## popan.func 19/9/14
        ...
        if(lambdamodel){
                ...
                replace.calc.func <- function(allvals){
                        ...
                        phival <- allvals[phinames][1]
                        ## Extract lambda:
                        lambdaval <- allvals["lambda"]
        ...
        negloglike.func <- function(pars, returnflag=F){
                ...
                ## values from pars:
                allvalues[inds.insert.into.allvalues] <- pars[which.pars.into.allvalues]
                ...
                ## Have found nlm performs better than nlminb, so we need to enforce parameter constraints by
                ## tricks here:
                ## Want to return a large positive value if any of the parameters are out of range.
                ## Safeguard this by including an option to return a flag of 1 in case the final returned estimates
                ## are based on parameters out of range or an infinite or NA nllike.  (The infinite or NA nllike is
                ## dealt with at the end of this function.)
                if(returnflag){
                        if((N<nhist) | any(c(pvec, phivec, pentvec)<0) | any(c(pvec, phivec, pentvec)>1)) return(1)
                }
                if(N<nhist) return(2000*abs(N+nhist))
                if(any(c(pvec, phivec, pentvec)<0)) return(20000*abs(min(c(pvec, phivec, pentvec))-1))
                if(any(c(pvec, phivec, pentvec)>1)) return(20000*abs(max(c(pvec, phivec, pentvec))+1))
                ## Lambdamodels must have lambda < phi, even though the lambda value is now absorbed into
                ## pentvec:
                if(lambdamodel)
                        if(allvalues["lambda"]<phivec[1]) return(20000*abs(allvalues["lambda"]-phivec[1]+1))
                ...
                for(i in (k-1):1) {
                        ...
                        new.phi.min.index <- phi.min.index - gapvec[i]
                        ...
                        chivec[i] <- 1 - psurvive.gap[i] + psurvive.gap[i] * (1-pvec[i+1]) * chivec[i+1]
                        phi.min.index <- new.phi.min.index
                }
                log.psurvive.gap <- log(psurvive.gap)
                ...
                nllike <- -lgamma(N + 1) + lgamma(N - nhist + 1) - (N - nhist) * log.p.unseen
                ...
                for(f in 1:k) if(any(first.obs==f)){
                        ...
                        psif.vec <- numeric(f)
                        psif.vec[f] <- 1
                        if(f > 1) for( i in (f-1):1) psif.vec[i] = (1 - pvec[i]) * psurvive.gap[i] * psif.vec[i+1]
                        ...
                        ##                prod_{i=F}^{L-1} [ p_i^delta_i  (1-p_i)^{1-delta_i} * psurvive.gap_i ]
                        ...
                        for(hst in 1:n.f) if(last.dat.f[hst] > f){
                                ...
                                log.psurvive.gap.hst <- log.psurvive.gap[f:(last.hst-1)]
                                ## The contribution to the log-likelihood from datvec.hst is
                                ## log { prod_{i=F}^{L-1} [ p_i^delta_i  (1-p_i)^{1-delta_i} * psurvive.gap_i ] }
                                ## = sum_{i=F}^{L-1} {  delta_i*log(p_i) + (1-delta_i)*log(1-p_i) + log(psurvive.gap_i)}
                                loglik.hst <- datvec.hst * log(p.hst) + (1 - datvec.hst) * log(1-p.hst) + log.psurvive.gap.hst
                                nllike <- nllike - sum(loglik.hst)
                        }

                }  ## End of f (likelihood for all animals with first sighting at occasion f).

                ## If things have gone wrong, give nllike a large positive value:
                if(returnflag){
                        ## If returning the flag that things have gone wrong, we have already dealt with
                        ## out-of-range parameters at the top of this function.  Deal with infinite or NA
                        ## nllike here:
                        if(is.na(nllike) | is.infinite(nllike)) return(1)
                        else return(0)
                }
                ## If not returnflag, continue by allotting nllike a ridiculously large value so the evaluations can continue:
        ...
        code <- mle.res$code  ## This should be 1 or 2, or maybe 3, for nlm, if all is well.
        npar <- length(mle.params)

        ## Check the flag of the final estimate:
        flag.est <- negloglike.func(mle.params, returnflag=T)
        
        # print(negloglike.func(c(startvals[1] * 2, rep(1, length(startvals) - 1))))
        # print(mle.params)
        

        ## Calculate the AIC:  AIC = 2*npar - 2 * max( log like )  and we want the minimum AIC.
        ## So AIC = 2*npar - 2 * (- min (-log like)) = 2*npar + 2*min(negloglike)
        min.negloglike <- mle.res$minimum
        ...
        varmat <- try(solve(mle.res$hessian))
        ...
        all.res <- c(mle.params, var.vec, min.nll=min.negloglike, npar=npar,
                     AIC=AIC, AICc=AICc, code=code, flag=flag.est, iter=mle.res$iterations)
        all.res
}
...
sim.func <- function(N, simtable){
        ...
        sim.ent <- rmultinom(N, size=1, prob=simtable$pent)
        time.ent <- apply(sim.ent, 2, function(x)which(x==1))
        ...
        for(i in 1:N) alive.dat[i, time.ent[i]:nt] <- 1

        ## Go through the columns of alive.dat one by one.  Everything with a 1 in a column of alive.dat
        ## can become 0 in the next column (failed to survive). Survival from time t to time t+1 is governed by
        ## simtable$phivec[t].
        for(t in 1:(nt-1)){
                inds.alive <- which(alive.dat[,t]==1)
                alive.next <- rbinom(length(inds.alive), 1, simtable$phivec[t])
                alive.dat[inds.alive, t+1]  <- alive.next
                ## Anything that died in t+1 now gets a 0 for the rest of the row.
                which.died <- inds.alive[alive.next==0]
                if(t+2 <= nt) alive.dat[which.died, (t+2):nt] <- 0
        }

        ## ------------------------------------------------------------------------------------------------------------
        ## Detection
        ##-------------------------------------------------------------------------------------------------------------
        ## Create avail.dat, which is the columns of alive.dat that correspond to surveys:
        avail.dat <- alive.dat[, simtable$survey]
        ## psurv are the capture probabilities corresponding to surveys:
        psurv <- simtable$pvec[simtable$survey]
        ## Go through avail.dat one column at a time and simulate detections of animals alive.
        ## Animals not alive automatically get 0 for detection.
        det.hist <- matrix(0, ncol=nsurv, nrow=N)
        for(t in 1:nsurv) det.hist[, t] <- rbinom(N, 1, psurv[t]) * avail.dat[,t]

        ## Delete animals that are never seen, to give the observed data:
        which.keep <- (1:N)[rowSums(det.hist)>0]
        det.dat <- det.hist[which.keep,]
