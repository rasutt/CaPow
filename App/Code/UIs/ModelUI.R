    ...
    div(style = "width:1070px;",
        tabsetPanel(
          tabPanel("Model Builder", {
            ## If previous model given, load that
            # if(exists("tempModel", envir = CPenv, inherits = FALSE)){
            #   tempModel = get("tempModel", envir = CPenv)
            #   varModelName = tempModel$modelname
            #   varDescription = tempModel$description
            #   varTimeN = nrow(tempModel$paramdf)
            #   varsuperpopn = tempModel$superpopn
            #   varModelType = tempModel$modeltype
            #   varlambdaparam = tempModel$lambdaparam
            #   if(varModelType=="lambdamodel" | varModelType=="singlephimodel"){
            #     varsinglephi = tempModel$paramdf$survrate[tempModel$paramdf$survrate!=""][1]
            #     vartimestart = tempModel$paramdf$timelabels[1]
            #   }
            #   else{
            #     varsinglephi = ""
            #     vartimestart = ""
            ...
            listModelType <- list("Lambda-POPAN" = "lambdamodel",
                                  "Standard POPAN, single Phi" = "singlephimodel",
                                  "Standard POPAN" = "fullmodel")
            ## The line below no longer applies: it used to be the case that when radiobuttons
            ## were defined, their selected choice used the name (e.g. Standard POPAN)
            ## rather than the ID (e.g. fullmodel).
            ## However this seems to have changed in later versions of Shiny.
            ## nameModelType <- names(listModelType)[listModelType==varModelType]
            ...
            tagList(  ## Beginning of overall display (left- and right-hand panels)
                  ...
                  conditionalPanel(ns = NS(id),
                                   ...
                                   div(style = "padding:10px; border:1px solid; width:450px; float:left;",
                                       ...
                                       tagList(
                                         ...
                                         #          label = "Time Labels", reverse = TRUE,
                                         #          checked = FALSE),
                                       ...
                                       tagList(
                                         ...
                                         #          label = "Survival Rate", reverse = TRUE,
                                         #          checked = FALSE),
              ...
              div(style = "padding:10px; border:1px solid; width:480px; float:left;",
                  ...
                  ## HTML("<strong>try this?</strong>")
                  ## HTML("<font color=\'#CC0033\'><strong>Type briskly when entering values directly into the matrix.</strong></font><p>"),
                  ## HTML("<font color=\'#666666\'>The system may update and reset your entry while you are typing. If you need to delete and re-enter more than two characters, consider using comma-separated entry in the Easy-Fill panel.</font>")
          ...
          tabPanel("View Models",
                   # HTML("This tab displays a reference list of Models already created. This list is <strong>not</strong> updated to reflect any changes made in the current session. It shows the Models as they were at the start of the session."),
          ...
          tabPanel("HELP : General",
                   ...
                   # h4("Entry and exit:"),
                   # HTML("You can start ModelBuilder to edit or copy an existing model.<ul><li>ModelBuilder() opens with default settings. By default, the interface will open with the most recent model you have been working on. If you don't want this, use ModelBuilder(reset=T). <li>ModelBuilder(\"M1\") opens model M1 for editing. <li>ModelBuilder(copy=\"M1\") copies model M1 to create a new model. The new model is based on model M1, but has a blank name to ensure model M1 is not accidentally overwritten.</ul><p>"),
                   # HTML("To finish, close the process in R using the Escape key or Stop button. (Closing your browser is not necessary and does not end the process in R.) The model that you have just been editing will be saved temporarily in draft form, so if your system crashes it will not be lost. However, it will be lost if you reopen the system with ModelBuilder(reset=T)."),
